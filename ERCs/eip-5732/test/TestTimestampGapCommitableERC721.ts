...
describe("Contract", function () {
  ...
  async function deployFixture() {
    ...
    const { contract, tx } = await deployByName(ethers, "TimestampGapCommitableERC721", [version]);
  ...
  describe("CommitToMint", function () {
    it("Should allow minting if has valid commitment and only with valid timestamp gaps", async function () {
        ...
        await expect(contract.safeMint(addr1.address, fakeTokenId, salt))
            .to.be.rejectedWith('Not enough commitment timestamp gap yet.');
